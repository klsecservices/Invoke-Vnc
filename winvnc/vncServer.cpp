...
vncServer::Authenticated(vncClientId clientid)
{
	...
	BOOL authok = TRUE;
	dprintf("inside vncserver::Authenticated");
	...
	for (i = m_unauthClients.begin(); i != m_unauthClients.end(); i++)
	{
		...
		if ((*i) == clientid)
		{
			dprintf("vncserver::Authenticated: before GetClient");
			...
			if (m_desktop == NULL)
			{
				...
				dprintf("after new vncDesktop. before desktop init");
				if (!m_desktop->Init(this))
				{
					dprintf("after new vncDesktop. Desktop init failed");
				...
				dprintf("after new vncDesktop. after desktop init");
			}
	...
	if (authok && GetBeepConnect())
	{
		...
		dprintf("at the end of vncserver::Authenticated");
