...
vncDesktopThread::Init(vncDesktop *desktop, vncServer *server)
{
	...
	// Start the thread
	dprintf("vncDesktopThread::Init before start_undetached");
	start_undetached();
...
{
	dprintf("vncDesktopThread::run_undetached befor getthreaddesktop(getcurrentthreadid))");
	// Save the thread's "home" desktop, under NT (no effect under 9x)
	HDESK home_desktop = GetThreadDesktop(GetCurrentThreadId());
	dprintf("vncDesktopThread::run_undetached GetThreadDesktop resulting handle: %d", home_desktop);
...
// TODO: review strong guarantee conditions for vncDesktop::Startup
	...
	dprintf("vncDesktopThread::run_undetached desktop->Startup() successful");

	RECT rect = m_desktop->GetSourceRect();
	dprintf("m_desktop address %p", m_desktop);
	...
	while (TRUE)
	{
		dprintf("AZAZA msg = %x", msg.message);
		//dprintf("vncDesktopThread::run_undetached inside while true loop");
		int peekres = PeekMessage(&msg, m_desktop->Window(), NULL, NULL, PM_REMOVE);
		dprintf("After PEEK msg = %x", msg.message);
		if(!peekres)
...
// TODO: suppress this message from hook when driver is active
		...
		{
			dprintf("Going to dispatch msg.message %x", msg.message);
...
vncDesktop::Startup()
{	
	dprintf("vncDesktop::Startup before checking inConsoleSession()");
	// Currently, we just check whether we're in the console session, and
	//   fail if not
	if (!inConsoleSession()) {
		dprintf("vncDesktop::Startup !inConsoleSession()");
		return FALSE;
	}
	...
	dprintf("vncDesktop::Startup before InitVideoDriver()");
...
//		InvalidateRect(NULL,NULL,TRUE);
	...
	dprintf("vncDesktop::Startup before PullFullScreen()");
	// Add the system hook
	//ActivateHooks();
...
// of clients when desktop is created.
	...
	dprintf("vncDesktop::Startup returning True");
	return TRUE;
}
...
BOOL vncDesktop::InitWindow()
{
	...
	dprintf("vncDesktop::InitWindow before CreateWindow()");
	// And create a window
	...
	dprintf("vncDesktop::InitWindow after CreateWindow()");

	if (m_hwnd == NULL) {

		dprintf("vncDesktop::InitWindow window handle is null");
		return FALSE;
	}
	dprintf("vncDesktop::InitWindow window handle is ok");

	// Set the "this" pointer for the window
	SetWindowLongPtr(m_hwnd, -21, (LONG_PTR)this); // #define GWL_USERDATA        (-21)
	dprintf("vncDesktop::InitWindow after SetWindowLong. m_hwnd: %d", m_hwnd);
	// Enable clipboard hooking
	m_hnextviewer = SetClipboardViewer(m_hwnd);
	dprintf("vncDesktop::InitWindow after SetClipboardViewer");
	return TRUE;
}
...
vncDesktop::Init(vncServer *server)
{
	...
	dprintf("vncDesktop::Init before LoadCursor");
	...
	if (thread == NULL)
		return FALSE;
	m_thread = thread;
	dprintf("vncDesktop::Init before thread init");
...
{
	vncDesktop *_this = (vncDesktop*)GetWindowLongPtr(hwnd, GWLP_USERDATA);// #define GWL_USERDATA        (-2
	dprintf("21GetWIndowLong %p", _this);
	...
	case WM_TIMER:
	 	dprintf("Case wm_timer. wParam = %d", wParam);
		switch (wParam) {
		case vncDesktop::TIMER_POLL:
			dprintf("before setplling");
			_this->SetPollingFlag(true);
			dprintf("after setplling");
			break;
		...
		dprintf("case wm_timer returning");
		return 0;
...
BOOL vncDesktop::InitVideoDriver()
{
	...
	dprintf("vncDesktop::InitVideoDriver after windows version checks");
	...
	_ASSERTE(!m_videodriver);
	dprintf("vncDesktop::InitVideoDriver after assert");
...
// NOTE that on NT4 it must be running beforehand
	...
	dprintf("vncDesktop::InitVideoDriver before if (!m_videodriver->CheckVersion())");
...
// IMPORTANT: fail on NT46
	...
	dprintf("vncDesktop::InitVideoDriver before if (m_videodriver->MapSharedbuffers(bSolicitDASD))");
	if (m_videodriver->MapSharedbuffers(bSolicitDASD))
	{
		dprintf("vncDesktop::InitVideoDriver m_videodriver->MapSharedbuffers(bSolicitDASD) TRUE");
	}
	else
	{
		dprintf("vncDesktop::InitVideoDriver m_videodriver->MapSharedbuffers(bSolicitDASD) FALSE");
		delete m_videodriver;
		m_videodriver = NULL;

		return FALSE;
	}
	dprintf("vncDesktop::InitVideoDriver after if (m_videodriver->MapSharedbuffers(bSolicitDASD))");
