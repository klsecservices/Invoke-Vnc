...
BOOL vncVideoDriver::MapSharedbuffers(BOOL fForDirectScreenAccess)
{
	...
	_ASSERTE(!m_fDirectAccessInEffect);
	_ASSERTE(IsWinNT());
	dprintf("vncVideoDriver::MapSharedbuffers trying CreateDC with m_devname: %s", m_devname);
	HDC	l_gdc= ::CreateDC(m_devname, NULL, NULL, NULL);
	//HDC	l_gdc= ::CreateDC(TEXT("DISPLAY"), NULL, NULL, NULL);
	if (!l_gdc)
	{
		dprintf("vncVideoDriver::MapSharedbuffers CreateDC returned NULL");
		//ExitThread(0);
		return FALSE;
	}
	dprintf("vncVideoDriver::MapSharedbuffers CreateDC returned HDC: %d", l_gdc);
