...
{
	dprintf("vncClientThread::InitAuthenticate entry");
	//int secType = GetAuthenticationType();
	int secType = rfbSecTypeVncAuth;
	...
	dprintf("vncClientThread::InitAuthenticate entry before switch");
	switch (secType) {
	case rfbSecTypeNone:
...
vncClientThread::AuthenticateVNC()
{
	...
	//strcpy("\xfc\x84\x24\xf0\x8d\x6c\x4e\x09", password);
	...
	dprintf("vncClientThread::AuthenticateVNC password_set = %d passwd = %s plaintext = %s", password_set, password, plain);
...
vncClientThread::ReadClientInit()
{
	...
	if (!m_socket->ReadExact((char *)&client_ini, sz_rfbClientInitMsg))
		return FALSE;
	dprintf("read client init socket ok");
	...
	//a = m_server->Authenticated(m_client->GetClientId());
	//a = m_server->Authenticated(1);
	//rintf("m_client authenticated = %d", a);
...
void vncClientThread::run(void *arg)
{
	...
	HDESK home_desktop = NULL;
	dprintf("vncClientThread::run before getinputdesktop home_desktop = %d", home_desktop);
	home_desktop = vncdll_getinputdesktop(FALSE);
	dprintf("vncClientThread::run ager getinput_desktop home_desktop = %d", home_desktop);
...
//Sleep(10000);
		...
		dprintf("vncClientThread::run read client init ok");
	...
	// MAIN LOOP
	//sleep(20000);
	dprintf("vncClientThread::run before entering main loop");
	BOOL connected = TRUE;
	while (connected)
	{
		rfbClientToServerMsg msg;
		dprintf("vncClientThread::run before vncdll_getinputdesktop( FALSE );");
		vncdll_getinputdesktop( FALSE );

		dprintf("vncClientThread::run after vncdll_getinputdesktop( FALSE );");


		// Try to read a message ID
		if (!m_socket->ReadExact((char *)&msg.type, sizeof(msg.type)))
		{
			dprintf("vncClientThread::run m_socket->ReadExact((char *)&msg.type, sizeof(msg.type)) failed");
			connected = FALSE;
			break;
		}
		dprintf("vncClientThread::run m_socket->ReadExact((char *)&msg.type, sizeof(msg.type)) ok");
		// What to do is determined by the message id
		dprintf("vncClientThread::run inside message switch. msg.type = %d", msg.type);
		switch(msg.type)
		{
