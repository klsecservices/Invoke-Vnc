...
HDESK vncdll_getinputdesktop( BOOL bSwitchStation )
{
	...
	dprintf("vncdll_getinputdesktop inside");
	do
	{
		if( bSwitchStation )
		{
...
DWORD vncdll_run( AGENT_CTX * lpAgentContext )
{
	...
	BYTE bFlags         = 0;
	char encrypted[8];
	vncEncryptPasswd(lpAgentContext->password, encrypted);
	...
	do
	{
		...
		vserver->SetPassword(TRUE, encrypted);
		...
		dprintf( "after wait for single object");
 ...
 */
AGENT_CTX * vncdll_command_dword( char * cpCommandLine, char * cpCommand )
{
	dprintf("%s %s", cpCommandLine, cpCommand);
	char * cpString = NULL;
	AGENT_CTX * dwResult  = 0;
	...
	do
	{
		...
		#ifdef _WIN64
		dwResult = (AGENT_CTX *) _strtoui64( cpString, NULL, 0 );
		#else
		dwResult = (AGENT_CTX *) strtoul(cpString, NULL, 0 );
		#endif


	} while( 0 );

	dprintf("Agent context address is %08p", dwResult);
	return dwResult;
}
...
BOOL WINAPI DllMain( HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved )
{
    ...
    { 
		case DLL_PROCESS_ATTACH:
			...
			dprintf("Inside winvnc dllmain. Not Sleeping");
			//turn TRUE;
			//Sleep(30);
			//return TRUE;
			if( lpReserved != NULL )
